...
// HACK: Relies on the fact that the board size is the same thoroughout the
// whole runtime. This is good enough for now, but **should** be refactored in
// the future.
...
impl Board {
  ...
  fn evaluate_sequence(&self, sequence: &[usize]) -> Eval {
    ...
    // All `get_unchecked` is encapsulated in `seq_tile` / `board_tile` — safe
    // because `sequence` is generated by `generate(size)` and validated in
    // `initialize_sequences` (`6*size-2` len check), so every `sequence[k]
    // < size*size == data.len()`.
    let seq_tile = |k: usize| unsafe {
      // SAFETY: k < n == sequence.len(), sequence[k] in-bounds for data
      *self.data.get_unchecked(*sequence.get_unchecked(k))
    };
    let board_tile = |idx: usize| unsafe {
      // SAFETY: idx comes from `sequence`, already validated as above
      *self.data.get_unchecked(idx)
    };

    let mut pos = 0;
    std::iter::from_fn(|| {
      let start = (pos..n).find(|&k| seq_tile(k).is_some())?;
      let player = seq_tile(start).expect("checked Some");
      let open_left = start > 0 && seq_tile(start - 1).is_none();

      let mut stones: u8 = 0;
      let mut has_hole = false;
      let mut end = start;

      for (current, &idx) in sequence.iter().enumerate().skip(start) {
        let tile = board_tile(idx);
        match tile {
          Some(p) if p == player => {
            stones += 1;
            end = current + 1;
          ...
          None
            if !has_hole
              ...
              && stones < 5
              && current + 1 < n
              && seq_tile(current + 1) == Some(player) =>
          {
            has_hole = true;
            end = current + 1;
          },
      ...
      let open_right = end < n && seq_tile(end).is_none();
      let open_ends = u8::from(open_left) + u8::from(open_right);
      let consecutive = stones + u8::from(has_hole);
      let (score, is_win) = shape_score(consecutive, open_ends, has_hole);

      pos = end.max(start + 1);
      Some((player, score, is_win))
    })
    .fold(Eval::default(), |mut acc, (player, s, win)| {
      acc.score[player] += s;
      acc.win[player] |= win;
      acc
    })
